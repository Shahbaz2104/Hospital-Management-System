...
### Session 12 — Phase 10 complete: Stripe payment automation + polish

**2026-08-09**

- **Stripe payment automation** (`65ad6d4`) — production-ready payments per user requirement:
  - **Checkout** — `POST /api/billing/invoices/[id]/checkout` creates a Stripe Checkout session (reuses open PENDING session; success_url `/payments?session=`); `GET .../pay-link` returns a shareable payment link. UI: "Pay online" + "Copy payment link" row/footer actions in billing page (hidden for CANCELLED/REFUNDED or zero balance).
  - **Webhooks** — `POST /api/webhooks/stripe` (raw body, signature-verified, idempotent): `checkout.session.completed` → payment COMPLETED + `recomputeInvoicePaid` (partial payments supported — invoice flips to PAID only when paid == total); `checkout.session.expired` → PENDING payment voided; `refund.created` → negative payment row + invoice REFUNDED when fully refunded. Middleware exempts the webhook path.
  - **Refunds** — refund dialog now calls Stripe Refunds for CARD payments (stored `stripeRefundId`), keeps in-app reversal for CASH/BANK; gateway errors → 502, unconfigured → 503.
  - **Receipts** — `GET /api/payments/[id]/receipt` returns a branded A4 PDF (pdf-lib) from `services/receipts.ts`; buttons in billing detail + payments page; route guarded `billing:read` so RECEPTIONIST can use it.
  - **Schema** — Payment gains `stripeSessionId`/`stripePaymentIntentId`/`stripeRefundId`/`stripeChargeId` (non-unique — Mongo unique indexes can't hold multiple nulls; idempotency enforced in code); `EmergencyCase.admittedAsAdmissionId` de-uniqued → one-to-many `Admission.emergencyCases`. `.env.example` documents `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`.
  - Same commit also delivered **Cloudinary** (`/api/upload` for logo + records, settings logo upload, record file attachments), **minimal PWA** (manifest, SW, offline page, generated icons), **a11y/perf** (`/` search shortcut, aria-labels on ~15 icon buttons), and **spec-scale seed** (150 patients / 30 doctors / 20 nurses / 10 depts / 100 medicines / 500 appointments / 200 invoices / 100 admissions / 50 lab orders, verified via counts) with bounded-parallel batches (25 concurrent) — cut seed runtime from >10 min timeout to ~2 min; attendance backfill dedupe window fixed.
...
- **Follow-up** — animated ECG trace component removed per user (`vitals-line.tsx` deleted; header divider + auth hero strip replaced with plain hairlines; dead CSS removed). Static ECG glyphs (sidebar brand chip, `app/icon.svg`, stat-card tick, loading mark) retained.
